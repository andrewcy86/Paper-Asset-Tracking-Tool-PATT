...
wp_register_script('customScriptDatatables', plugins_url('js/customScriptDatatables.js', dirname(__FILE__), '', true));
...
WHERE wpqa_wpsc_epa_boxinfo.ticket_id = 0000001-2"// . $ticket_id_val
);*/
...
INNER JOIN wpqa_epa_record_schedule ON wpqa_wpsc_epa_boxinfo.record_schedule_id = wpqa_epa_record_schedule.id
WHERE wpqa_wpsc_epa_boxinfo.box_id = '" . $id . "'";*/
 $box_details = $wpdb->get_row("SELECT * FROM patt_andrew.wpqa_wpsc_epa_boxinfo");
 $box_content = $wpdb->get_results("SELECT * FROM patt_andrew.wpqa_wpsc_epa_folderdocinfo");
echo "<br>";
print_r($box_details);
echo "<br> hello";
print_r($box_details->box_id);
      ...
      echo "<h3>Box</h3>";
      echo "<strong>Box ID:</strong> " . $box_details->box_id . "<br />";
...
$dash_count=array_rand($a,3);


if ($dash_count/*preg_match("/^[0-9]{7}$/", $id) || preg_match("/^[0-9]{7}-[0-9]{1,4}$/", $id) || preg_match("/^[0-9]{7}-[0-9]{1,4}-[0-9]{2}-[0-9]{1,4}$/", $id)*/) {
  switch ($dash_count) {
    case 0:
