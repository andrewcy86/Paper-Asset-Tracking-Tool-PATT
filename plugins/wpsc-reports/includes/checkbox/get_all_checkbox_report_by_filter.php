...
if($date_filter == 'last7days'){
   ...
   include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_last_7.php' );
  
}else if($date_filter == 'last30days'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_last_30.php' );

}else if($date_filter == 'lastmonth'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_last_month.php' );
  
}else if($date_filter == 'lastquarter'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_last_quarter.php' );
  
}else if($date_filter == 'thisyear') {
  
  include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_this_year.php' );
  
}else if($date_filter == 'customdate') {
  
  include_once( WPSC_RP_ABSPATH . 'includes/checkbox/report_ck_custom_date.php' );
...
if(array_sum($sumArray) == 0){
  ...
  <?php
    ...
    ?>
      ...
      <div style="width:100%;">
        <canvas id="ticket_checkbox"></canvas>
      </div>
      ...
      jQuery(document).ready(function() {
        var checkbox = document.getElementById('ticket_checkbox').getContext('2d');
        window.myHorizontalBar = new Chart(checkbox, config);
      });
      </script>
    <?php
    }
