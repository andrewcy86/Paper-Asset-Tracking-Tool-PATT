...
$last_60_31_no_tickets = $wpdb->get_var("SELECT SUM(ticket_count) FROM {$wpdb->prefix}wpsc_reports  WHERE result_date BETWEEN '".$last_60_31_first."' AND '".$last_60_31_last."' AND report_type='no_of_tickets' ");

if($no_of_tickets > $last_60_31_no_tickets){
  if($last_60_31_no_tickets > 0) {
    $no_of_tic_percentage = $no_of_tickets - $last_60_31_no_tickets;  
  }else{
    $no_of_tic_percentage = $no_of_tickets;  
  }
  $ticket_graph = 'increasing';
}elseif($no_of_tickets == $last_60_31_no_tickets){
  ...
  }elseif($last_60_31_no_tickets == 0 && $no_of_tickets == 0){
    $no_of_tic_percentage = '';
    $ticket_graph =  '';
  }else{
    $no_of_tic_percentage = $last_60_31_no_tickets;
    $ticket_graph =  'decreasing';
  }
  
}

$last_30_days_avg_in_min = $wpdb->get_var("SELECT AVG(ticket_count) FROM {$wpdb->prefix}wpsc_reports WHERE result_date BETWEEN '" . $first . "' AND '" . $last . "' AND report_type ='first_response' AND ticket_count !=0 ");
...
$average = $last_30_days_avg_in_min/60 + $today_frt_avg;
...
$last_60_31_average_in_min = $wpdb->get_var("SELECT AVG(ticket_count) FROM {$wpdb->prefix}wpsc_reports WHERE result_date  BETWEEN  '" . $last_60_31_first . "'  AND  '" . $last_60_31_last . "' AND report_type = 'first_response' AND ticket_count !=0");

$last_60_31_average = $last_60_31_average_in_min/60;

if($average > $last_60_31_average ){
  if($last_60_31_average > 0) {
    $frt_percentage = $average - $last_60_31_average;  
  }else{
    $frt_percentage = $average;
  }
  $frt_graph = 'increasing';
}elseif($average == $last_60_31_average){
  $frt_percentage = '';
  $frt_graph =  '';
}else{
  if($last_60_31_average > 0 && $average > 0 ){
    $frt_percentage = $average - $last_60_31_average; 
    $frt_graph =  'decreasing';
  }elseif($last_60_31_average == 0 && $average == 0){
    $frt_percentage = '';
    $frt_graph =  '';
  }else{
    $frt_percentage = $last_60_31_average;
    $frt_graph =  'decreasing';
  }
}
