...
if($date_filter == 'last7days'){
  ...
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_last_7.php' );

}else if($date_filter == 'last30days'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_last_30.php' );

}else if($date_filter == 'lastmonth'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_last_month.php' );
  
}else if($date_filter == 'lastquarter'){
  
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_last_quarter.php' );
  
}else if($date_filter == 'thisyear') {
  
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_this_year.php' );
  
}else if($date_filter == 'customdate') {
  
  include_once( WPSC_RP_ABSPATH . 'includes/dropdown/report_td_custom_date.php' );
...
if(array_sum($sumArray) == 0){
  ...
  ?>
    ...
    <div style="width:100%;">
      <canvas id="ticket_dropdown"></canvas>
    </div>
    ...
    jQuery(document).ready(function() {
      var dropdown = document.getElementById('ticket_dropdown').getContext('2d');
      window.myHorizontalBar = new Chart(dropdown, config);
    });
    </script>
  <?php
  }
