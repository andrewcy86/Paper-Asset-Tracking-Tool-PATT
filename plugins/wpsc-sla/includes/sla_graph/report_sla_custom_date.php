...
if($days<=30){
  ...
  foreach ($week_array as $key => $value) {
      ...
      $count    = $wpdb->get_var("SELECT overdue_count FROM {$wpdb->prefix}wpsc_sla_reports WHERE result_date BETWEEN  '".$value['start']."' AND '".$value['end']."'");
  ...
  foreach ($mdate_array as $key => $month) {
    ...
    $count = $wpdb->get_var("SELECT overdue_count FROM {$wpdb->prefix}wpsc_sla_reports WHERE result_date BETWEEN  '".$month['start']."' AND '".$month['end']."'");
